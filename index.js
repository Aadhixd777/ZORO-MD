...
async function downloadAndExtractModules() {
    ...
    try {
        ...
        // settings.js ലെ ZORO-MD മൊഡ്യൂൾ ഫോൾഡർ ഓട്ടോമാറ്റിക്കായി കണ്ടുപിടിക്കുന്നു
        const moduleFolder = extractedFolders.find(f => 
            f.includes('ZORO-MD-MODULES') || 
            f.includes('NAGIIP-STAR-MD-MODULES') || 
            f.includes('main') || 
            fs.statSync(path.join(EXTRACT_DIR, f)).isDirectory()
        );
...
async function checkAndInstallFFmpeg() {
    ...
    try {
        ...
        // Cleanup - Safe unlink using existsSync check
...
async function startBot() {
    ...
    app.get('/', (req, res) => res.send('ZORO MD Bot is Alive!'));
    app.listen(port, () => console.log(`🚀 Keep-alive server running on port ${port}`));

    const settings = require('./settings');

    console.log('\n╔════════════════════════════════════╗');
    console.log(`║  🚀 ${settings.botName || 'ZORO MD'} BOT STARTING...     ║`);
    console.log('╚════════════════════════════════════╝\n');
    ...
    // Safe import for main.js modules
    ...
    try {
        ...
        console.error("⚠️ Error loading main.js module:", e.message);
    }
    ...
    // Safe Import lightweight store
    let store = { readFromFile: () => {}, writeToFile: () => {}, bind: () => {}, contacts: {}, loadMessage: async () => null };
    try {
        store = require('./lib/lightweight_store');
        store.readFromFile();
    } catch (e) {
        console.log("⚠️ Lightweight store not found or failing, using fallback.");
    }
    
    setInterval(() => { if (store.writeToFile) store.writeToFile(); }, settings.storeWriteInterval || 10000);

    // Safe Import message queue for offline handling
    let messageQueue = { setConnected: () => {}, processQueue: async () => {}, addMessage: () => {} };
    try {
        const MessageQueue = require('./lib/messageQueue');
        messageQueue = new MessageQueue();
    } catch (e) {
        console.log("⚠️ MessageQueue not found, proceeding without message queueing.");
    }
    ...
    let phoneNumber = settings.ownerNumber || "918136880986";
    let owner = settings.botOwner || "Aadhixd";
    if (fs.existsSync('./data/owner.json')) {
        try { owner = JSON.parse(fs.readFileSync('./data/owner.json')); } catch {}
    }

    global.botname = settings.botName || "ZORO MD";
    global.themeemoji = "•";
    ...
    async function startXeonBotInc() {
        ...
        if (store.bind) store.bind(XeonBotInc.ev);
        ...
        XeonBotInc.sendMessage = async function(jid, content, options = {}) {
            try {
                ...
                if (messageQueue.addMessage) messageQueue.addMessage(jid, content, 1);
                throw error;
            }
        };
        ...
        XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
            try {
                ...
                try {
                    if (handleMessages) await handleMessages(XeonBotInc, chatUpdate, true);
                } catch (err) {
        ...
        XeonBotInc.public = settings.commandMode === "public";
...
https://chat.whatsapp.com/IUe14A04uicGJdIOfBuuvd?s=cl&p=a&ilr=1`,
                ...
                console.log(chalk.yellow(`\n\n╭━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╮`));
                console.log(chalk.bold.blue(`│     🔥 ${settings.botName || 'ZORO MD'} BOT 🔥     │`));
                console.log(chalk.yellow(`╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n`));
                
                console.log(chalk.cyan(`╔════════════════════════════════════╗`));
                console.log(chalk.green(`║  ✅ CONNECTION SUCCESSFUL! ✅     ║`));
                console.log(chalk.cyan(`╠════════════════════════════════════╣`));
                console.log(chalk.magenta(`║ 👤 Owner: ${settings.botOwner || 'Aadhixd'}               ║`));
                console.log(chalk.magenta(`║ 📱 Number: ${settings.ownerNumber || phoneNumber}         ║`));
                console.log(chalk.magenta(`║ 💎 Version: ${settings.version || '1.0.0'}                    ║`));
                console.log(chalk.magenta(`║ ⏰ Time: ${new Date().toLocaleString()}  ║`));
                console.log(chalk.magenta(`║ 🔥 Status: ON FIRE!                ║`));
                console.log(chalk.cyan(`╚════════════════════════════════════╝\n`));
                
                console.log(chalk.green(`${global.themeemoji || '•'} 🍁 ${settings.botName || 'ZORO MD'} is on fire 🔥`));
                console.log(chalk.blue(`${global.themeemoji || '•'} All systems operational!`));
            }
            if (connection === 'close') {
                if (messageQueue.setConnected) messageQueue.setConnected(false);
        ...
        XeonBotInc.ev.on('messages.upsert', async (m) => {
            if (m.messages[0].key && m.messages[0].key.remoteJid === 'status@broadcast') {
                if (handleStatus) await handleStatus(XeonBotInc, m);
            }
        });

        XeonBotInc.ev.on('status.update', async (status) => {
            if (handleStatus) await handleStatus(XeonBotInc, status);
        });
