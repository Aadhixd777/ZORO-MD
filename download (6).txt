...
// 🔥 FIX: Auto-create lib/myfunc if missing - prevents MODULE_NOT_FOUND
...
`;
    fs.writeFileSync(myfuncPath, fallbackMyfunc);
    console.log('✅ lib/myfunc.js created');
}
...
async function downloadAndExtractModules() {
    ...
    try {
        ...
        if (fs.existsSync(flag) && libExists) {
            console.log('✅ Modules already installed');
            return true;
        }
        if (fs.existsSync(flag) && !libExists) {
            console.log('⚠️ Flag exists but lib missing, re-downloading...');
